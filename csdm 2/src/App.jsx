...
function HomeScreen({ playerId, onEnter }) {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(r, snap => {
      ...
      setPublicRooms(Object.values(snap.val()).filter(r => r.phase === 'lobby' && r.playerCount < 12))
    })
    return () => off(r)
  }, [mode])
  ...
  async function handleCreate(isPublic = false) {
    ...
    const room = {
      code: roomCode, phase: 'lobby', hostId: playerId, isPublic,
    ...
    if (isPublic) await set(ref(db, `public_rooms/${roomCode}`), { code: roomCode, host: name.trim(), phase: 'lobby', playerCount: 1, createdAt: Date.now() })
  ...
  async function handleJoin(codeOverride) {
    ...
    await set(ref(db, `rooms/${joinCode}/players/${playerId}`), { id: playerId, name: name.trim(), hand, score: 0, submitted: false, submittedCard: null, vote: null, isHost: false })
    if (room.isPublic) await update(ref(db, `public_rooms/${joinCode}`), { playerCount: Object.keys(room.players||{}).length + 1 })
  ...
  return (
    <div style={s.page}>
      <div style={s.homeCard}>
        ...
        <p style={{ color:C.muted, textAlign:'center', fontSize:13, marginBottom:20, marginTop:8 }}>
        ...
        {mode === 'create' && <>
          <button style={{...s.btnPrimary, opacity:loading?.5:1}} disabled={loading} onClick={() => handleCreate(false)}>
            Crear sala privada
          </button>
          <button style={{...s.btnSecondary, opacity:loading?.5:1}} disabled={loading} onClick={() => handleCreate(true)}>
            🌐 Crear sala pública
          </button>
        </>}
        ...
        {mode === 'join' && <>
          <input style={{...s.input, marginTop:10, letterSpacing:4, textTransform:'uppercase'}}
            ...
            maxLength={5} />
          <button style={{...s.btnPrimary, opacity:loading?.5:1}} disabled={loading} onClick={() => handleJoin()}>
        ...
        {mode === 'public' && (
          <div style={{ display:'flex', flexDirection:'column', gap:8, marginTop:4 }}>
            {publicRooms.length === 0
              ? <div style={s.emptyBox}>No hay salas públicas abiertas.<br/><span style={{color:C.bluePale}}>¡Creá una vos!</span></div>
              : publicRooms.map(r => (
...
function GameScreen({ roomCode, playerId, onLeave }) {
  ...
  async function submitCard() {
    ...
    if (willSubmitted.length === pList.length) {
      updates.submissions = shuffle(pList.map(p => ({
  ...
  async function voteCard(targetPlayerId) {
    if (!me || me.vote !== null || targetPlayerId === playerId || votingFor) return
    setVotingFor(targetPlayerId)
    try {
      const snap = await get(ref(db, `rooms/${roomCode}`))
      const r    = snap.val()
      if (r.players?.[playerId]?.vote !== null) return
      const pList    = Object.values(r.players)
      const updVotes = { ...(r.votes||{}), [targetPlayerId]: ((r.votes||{})[targetPlayerId]||0) + 1 }
      const updates  = { [`players/${playerId}/vote`]: targetPlayerId, votes: updVotes }

      const votedAfter = pList.filter(p => p.vote !== null || p.id === playerId)
      if (votedAfter.length === pList.length) {
        ...
        updates.roundWinner = allSubs.find(s => s.playerId === winnerId) || null
        ...
        pList.forEach(p => {
          const h = p.id === playerId ? newHand : (p.hand||[])
          const needed = 10 - h.length
          if (needed > 0) { updates[`players/${p.id}/hand`] = [...h, ...deck.splice(0, needed)] }
        })
  ...
  const newHand = me ? me.hand.filter((_, i) => i !== selectedIdx) : []

  async function nextRound() {
    const snap = await get(ref(db, `rooms/${roomCode}`))
    const r    = snap.val()
    let blackDeck = Array.isArray(r.blackDeck) && r.blackDeck.length > 0 ? r.blackDeck : shuffle(BLACK_CARDS)
      ...
      <div style={s.page}>
        ...
        <div style={s.body}>
          ...
          <div style={s.blackCardSmall}>
            ...
            <p style={{color:'#fff', fontSize:16, fontWeight:700, lineHeight:1.4, margin:'8px 0 0'}}>
              {room.currentBlack}
            </p>
          </div>
          ...
          <div style={s.voteStatus}>
            {myVote === null
              ? <span>👇 Votá la respuesta que te parece la más graciosa</span>
              : <span>✅ Voto registrado — {votedCount}/{players.length} votaron</span>
            }
          </div>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:12}}>
            {allSubs.map((sub) => {
              ...
              const canVote  = myVote === null && !isMine && !votingFor
              const isLoading = votingFor === sub.playerId

              return (
                <div key={sub.playerId} style={{
                  ...s.voteRow,
                  ...(iVoted ? s.voteRowVoted : {}),
                  ...(isMine ? s.voteRowMine  : {}),
                }}>
                  {/* Left: who + their answer */}
                  ...
                  <div style={{flexShrink:0, marginLeft:12}}>
                    {isMine ? (
                      ...
                      <button
                        ...
                        style={{
                          ...s.voteBtn,
                          opacity: (!canVote || myVote !== null) ? 0.4 : 1,
                          background: isLoading ? C.muted : C.blue,
                        }}
                      >
                        {isLoading ? '…' : 'VOTAR'}
                      </button>
          ...
          {myVote !== null && (
            <div style={{color:C.muted, textAlign:'center', fontSize:13, marginTop:16}}>
        ...
        {iSubmitted ? (
          ...
          <>
            ...
            <div style={s.handGrid}>
              {(me?.hand || []).map((card, i) => (
                <HandCard key={i} text={card}
                  selected={selectedIdx === i}
                  onSelect={() => setSelectedIdx(selectedIdx === i ? null : i)}
                  onEdit={newText => {
                    const newH = [...(me.hand||[])]; newH[i] = newText
                    update(ref(db, `rooms/${roomCode}/players/${playerId}`), { hand: newH })
                  }} />
              ))}
            </div>
            <button style={{...s.btnPrimary, opacity: selectedIdx===null ? 0.35 : 1}}
              disabled={selectedIdx === null} onClick={submitCard}>
              Enviar carta
            </button>
          </>
        )}
...
const s = {
  ...
  tabs: {
    display:'flex', gap:6, margin:'14px 0',
  ...
  btnSecondary: {
    ...
    fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit', marginTop:8,
  },
  ...
  shareBox: { background:'rgba(251,191,36,0.08)', border:`1px solid ${C.gold}33`, borderRadius:12, padding:14, fontSize:14, color:C.text, lineHeight:1.6, textAlign:'center', marginBottom:16 },
  ...
  header: {
    ...
    gap:12, flexWrap:'wrap', marginBottom:8, position:'sticky', top:0, zIndex:50,
  },
  ...
  blackCardSmall: { background:'#080808', border:'2px solid #1a1a2e', borderRadius:12, padding:'16px 20px', marginBottom:16, boxShadow:'0 8px 24px rgba(0,0,0,0.5)' },

  // Vote rows
  voteStatus: {
    background:C.blueFaint, border:`1px solid ${C.border}`,
    borderRadius:10, padding:'12px 16px', fontSize:14,
    color:C.bluePale, textAlign:'center', marginBottom:16, fontWeight:600,
  },
  voteRow: {
  ...
  myCardTag: {
    background:C.gold, color:'#000', fontSize:11,
    fontWeight:800, padding:'6px 12px', borderRadius:8, letterSpacing:1,
  },
  votedTag: {
    background:C.blue, color:'#fff', fontSize:11,
    fontWeight:800, padding:'6px 12px', borderRadius:8, letterSpacing:1,
  },
